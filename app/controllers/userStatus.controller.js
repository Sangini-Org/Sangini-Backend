...
exports.addStatus = async (req, res) => {
  try {
    ...
    return sendBadRequest(
      res,
      404,
      "Error while creating Status " + err.message
    );
  }
};
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    return sendBadRequest(
      res,
      500,
      "Error while updating status" + err.message
    );
  }
};
exports.deleteStatus = async (req, res) => {
  try {
    ...
    return sendBadRequest(
      res,
      404,
      "Error while removing Status " + err.message
    );
  }
};
...
exports.getAllStatus = async (req, res) => {
  try {
    ...
    if (username) {
      ...
      if (!usersDetails.length) {
        return sendJSONResponse(
          res,
          200,
          "user status not found by username",
          []
        );
      }
      for (let user of usersDetails) {
        if (user.dataValues.id == req.id) {
          continue;
        }
        similarUsernameDetails.push(user.dataValues.id);
      }
    }
    const condition = similarUsernameDetails.length
      ? { userId: similarUsernameDetails }
      : null;
    ...
    return sendBadRequest(
      res,
      500,
      "Error while getting all status " + err.message
    );
  }
};
...
exports.getStatusById = async (req, res) => {
  try {
    ...
    return sendBadRequest(
      res,
      500,
      "error while getting status by id " + err.message
    );
  }
};
